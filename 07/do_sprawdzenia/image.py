...
class Images():
    ...
    def hough(self):
        ...
        accum, theta, rho = self.hough_calc(image)
        plt.imshow(accum, cmap='gray', extent=[np.rad2deg(theta[-1]),np.rad2deg(theta[0]), rho[-1], rho[0]],aspect='auto')
        plt.title("Hough")
        plt.show()

    def hough_calc(self,image):
        width, height = image.shape
        theta = np.deg2rad(np.arange(-90,90,1))
        diag = int(round(math.sqrt(width*width+height*height)))
        rho = np.linspace(-diag,diag,diag*2)
        cos = np.cos(theta)
        sin = np.sin(theta)
        len_theta = len(theta)
        accum = numpy.zeros((2*diag,len_theta),dtype=np.uint8)
        edge = image > 0
        ys, xs = numpy.nonzero(edge)

        for i in range(len(xs)):
            x = xs[i]
            y = ys[i]
            for j in range(len_theta):
                rho_loops = diag + int(round(x * cos[j] +y * sin[j]))
                accum[rho_loops, j] += 1
        return  accum, theta, rho
